...
async def process_single_person(llm: LlmClient, db: AsyncDatabaseManager, person_id: int, worker_id: int) -> bool:
    ...
    for attempt in range(config.MAX_RETRIES):
        logger.debug(f"[Воркер #{worker_id}][person_id={person_id}] ▶️ Обработка LLM. Попытка {attempt + 1}/{config.MAX_RETRIES}")
...
async def run(worker_id: int, person_id: int):
    ...
    logger.debug(f"[Воркер #{worker_id}][person_id={person_id}] ▶️ Начинаем LLM обработку")
